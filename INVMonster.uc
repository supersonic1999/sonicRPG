class INVMonster extends xPawn;
...
function myFire( optional float F )
{
    ...
    Controller.Target = Controller.PickTarget(bestAim, bestDist, vector(Controller.Rotation), GetFireStart(X,Y,Z), 6000);
    RangedAttack(Controller.Target);
}

function myAltFire( optional float F )
{
    local float bestAim, bestDist;
    local vector X,Y,Z;
    ...
    GetAxes(Controller.Rotation,X,Y,Z);
    Controller.Target = Controller.PickTarget(bestAim, bestDist, vector(Controller.Rotation), GetFireStart(X,Y,Z), 6000);
    AltRangedAttack(Controller.Target);
}
...
function Tick(float DeltaTime)
{
    local Inventory Inv;
    local int i;
    local array<Inventory> RemovedInvs;

    for(Inv=Inventory;Inv!=none;Inv=Inv.Inventory)
        if(Weapon(Inv) != none && TransLauncher(Inv) == none
        && TradeWeapon(Inv) == none && MonsterWeapon(Inv) == none)
            RemovedInvs[RemovedInvs.length] = Inv;

    for(i=0;i<RemovedInvs.length;i++)
        RemovedInvs[i].Destroy();
...
defaultproperties
{
     ...
     RequiredEquipment(0)="SonicRPG45.MonsterWeapon"
     bCanCrouch=False
